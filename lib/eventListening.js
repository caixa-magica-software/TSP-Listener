  ...
  econtract.events.AddedDocument({
    ...
    if (err) {
      ...
      if (result !== undefined) {
        ...
        let to_store = {
          'hash': web3.utils.hexToString(result.returnValues[0]),
          'hash_metamask': web3.utils.hexToString(result.returnValues[1]),
          'metamask_pubkey': web3.utils.hexToString(result.returnValues[2]),
          'wallet_address': result.returnValues[3],
          'document_id': web3.utils.hexToString(result.returnValues[4])
        };

        storeTSP(to_store, function(err, result) {
          if (err) {
            console.error('ERROR storing data in TSP: ', err);
